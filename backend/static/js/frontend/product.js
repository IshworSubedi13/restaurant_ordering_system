...
async function initCategories() {
  try {
    ...
    specialBtn.className = "category-btn";
    specialBtn.id = "special-filter-btn";
    specialBtn.dataset.category = "special";
    specialBtn.textContent = "Today's Special";
    menuCategoriesContainer.appendChild(specialBtn);
    ...
    console.error(error);
    menuCategoriesContainer.innerHTML = "<p class='error'>Error loading categories</p>";
  }
}
...
function renderMenuItems(category) {
  ...
  if (category === "all") {
    ...
    filteredItems = menuItems.filter((item) => item.is_special === true);
    if (filteredItems.length === 0) {
      menuItemsContainer.innerHTML = `
        <div class="no-special-items">
          <i class="fas fa-utensils"></i>
          <h3>No Specials Today</h3>
          <p>Check back tomorrow for new special offers!</p>
        </div>
      `;
      return;
    }
  } else {
    filteredItems = menuItems.filter((item) => slugify(item?.category?.name) === category);
  }
  filteredItems.forEach((item) => {
    ...
    if (item.is_special) {
      menuItemElement.classList.add("special-item");
    }
